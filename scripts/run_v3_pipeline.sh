...
# run_v3_pipeline.sh
#
# Runs scripts 09→10→11→12 on the V3 embeddings, then preserves all
# V3 outputs with "_v3" suffixes so you don't lose your V1 baseline.
#
# WHY THIS IS COMPLEX
# -------------------
...
# So we need to physically swap V3 files in, run the pipeline, then
# move outputs to V3-suffixed names and restore V1 files.
#
...
echo "============================================================"
echo "       Running V3 pipeline (canonical + IDF embedding)"
echo "============================================================"

# ─── Sanity checks ───────────────────────────────────────────────────────────
for f in \
    "$EMB/reference_emb_v3.npy" \
    "$EMB/rescue_emb_v3.npy" \
    "$EMB/reference_ids_v3.tsv" \
    "$EMB/rescue_ids_v3.tsv" ; do
    if [ ! -f "$f" ]; then
        echo "ERROR: $f not found. Run 08c_compute_embeddings_v3.py first."
        exit 1
    fi
done
...
# ─── Step 3: promote V3 files to the canonical names ─────────────────────────
echo ""
echo "[3/6] Promoting V3 files to canonical names..."
mv "$EMB/reference_emb_v3.npy" "$EMB/reference_emb.npy"
mv "$EMB/rescue_emb_v3.npy"    "$EMB/rescue_emb.npy"
mv "$EMB/reference_ids_v3.tsv" "$EMB/reference_ids.tsv"
mv "$EMB/rescue_ids_v3.tsv"    "$EMB/rescue_ids.tsv"
echo "      done."
...
# ─── Step 5: rename all V3 outputs and the V3 inputs to *_v3 names ───────────
echo ""
echo "[5/6] Saving V3 outputs with _v3 suffix..."

# Rename V3 input embeddings back
mv "$EMB/reference_emb.npy"  "$EMB/reference_emb_v3.npy"
mv "$EMB/rescue_emb.npy"     "$EMB/rescue_emb_v3.npy"
mv "$EMB/reference_ids.tsv"  "$EMB/reference_ids_v3.tsv"
mv "$EMB/rescue_ids.tsv"     "$EMB/rescue_ids_v3.tsv"

# Rename V3 outputs
mv "$IDX/faiss_pq.index"                 "$IDX/faiss_pq_v3.index"
mv "$IDX/faiss_opq.index"                "$IDX/faiss_opq_v3.index"
mv "$RES/retrieval_hits/hits_pq.tsv"     "$RES/retrieval_hits/hits_pq_v3.tsv"
mv "$RES/retrieval_hits/hits_opq.tsv"    "$RES/retrieval_hits/hits_opq_v3.tsv"
mv "$RES/rescue_predictions_pq.tsv"      "$RES/rescue_predictions_pq_v3.tsv"
mv "$RES/rescue_predictions_opq.tsv"     "$RES/rescue_predictions_opq_v3.tsv"
mv "$RES/metrics"                        "$RES/metrics_v3"
...
echo "V3 files: _v3 suffix (e.g. embeddings/reference_emb_v3.npy)"
echo ""
echo "Now run:  python scripts/compare_versions.py"
