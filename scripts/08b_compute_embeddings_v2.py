...
INPUT/OUTPUT NAMING
-------------------
V2 outputs go to embeddings/*_v2.npy etc. so they don't overwrite V1.
...
OUTPUTS
-------
    embeddings/reference_emb_v2.npy        (3.7M × 512 float32)
    embeddings/rescue_emb_v2.npy           (2.7M × 512 float32)
    embeddings/reference_ids_v2.tsv        (same content as V1)
    embeddings/rescue_ids_v2.tsv           (same content as V1)
    embeddings/idf_weights_v2.npy          (512-dim float32)

USAGE
-----
    conda activate bioenv
    cd ~/CompressedRescue
    python scripts/08b_compute_embeddings_v2.py
"""
...
REF_EMB_OUT       = EMB_DIR / "reference_emb_v2.npy"
RESCUE_EMB_OUT    = EMB_DIR / "rescue_emb_v2.npy"
REF_IDS_OUT       = EMB_DIR / "reference_ids_v2.tsv"
RESCUE_IDS_OUT    = EMB_DIR / "rescue_ids_v2.tsv"
IDF_OUT           = EMB_DIR / "idf_weights_v2.npy"

K                 = 5      # k-mer length (same as V1 — keep apples-to-apples)
DIM               = 512    # embedding dimension (same as V1)
HASH_SEED         = 0xC0FFEE
# ──────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    print("=" * 70)
    print("Next: bash scripts/run_v2_pipeline.sh")
